...
async function main() {
    ...
    const TEST_SEASON_DURATION = 30 * 60;
    ...
    const dividendsPool = await DividendsPool.deploy(feeReceiver,20, 10 ,30 ,100 ,foxToken.address ,fUSD_COIN_ADDR);
    await dividendsPool.deployed();
    console.log(`DividendsPool deployed to ${dividendsPool.address}`);

    const Airdrop = await ethers.getContractFactory("Airdrop");
    const airdrop = await Airdrop.deploy(ONE_YEAR_IN_SECS, TEST_SEASON_DURATION ,foxToken.address);
